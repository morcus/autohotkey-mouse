...
A separate chord, `~lbutton & rbutton::`, holds Ctrl+Shift while the left button is down to
drag windows between **PowerToys FancyZones** — the `~` keeps the physical left-click passing
through to the OS.

### Conventions when editing

- The three prefix layers are organized as labeled sections; keep new bindings inside the
  matching section and aligned with the existing tab-aligned trailing `;` comments describing
  the resulting action.
- Superseded bindings are kept as `; Deprecated` comment blocks rather than deleted — follow
  that pattern instead of removing old mappings outright.
